...
const { normalizeEmail, isProtectedEmail } = require('../auth/protected-accounts');
...
const rowToEmployee = (row) => {
  ...
  return {
    ...
    canManageInventory: Number(row.can_manage_inventory) === 1 || Number(row.can_view_stock) === 1,
    canViewHistory: Number(row.can_view_history) === 1 || Number(row.can_view_stock) === 1,
