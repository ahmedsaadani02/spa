...
export class StockStorageService {
    ...
    async getMovementsByMonth(monthISO) {
    ...
    async addMovement(movement) {
        ...
        const store = tx.objectStore(this.movementsStore);
        await this.requestToPromise(store.add(movement));
        await this.transactionDone(tx);
    }
    async applyMovement(item, movement) {
        ...
        await this.requestToPromise(items.put(item));
        await this.requestToPromise(movements.add(movement));
        await this.transactionDone(tx);
    }
    async ensureSeed() {
    ...
    ensureSerie40Accessories(items) {
        const now = new Date().toISOString();
        const next = [...items];
    ...
    dedupeSerie40Accessories(items) {
        const output = [];
    ...
    mergeStockItems(base, extra) {
        const quantities = { ...base.quantities };
        Object.keys(extra.quantities).forEach((color) => {
    ...
    applySeries67Normalization(items) {
        const now = new Date().toISOString();
        const deduped = [];
        const seen = new Set();
        ...
        const upsert = (template, requiredColors) => {
            const key = `${template.serie}|${template.category}|${template.reference}`.toLowerCase();
            const index = normalized.findIndex((item) => `${item.serie}|${item.category}|${item.reference}`.toLowerCase() === key);
    ...
    static { this.ɵfac = function StockStorageService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || StockStorageService)(); }; }
    static { this.ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: StockStorageService, factory: StockStorageService.ɵfac, providedIn: 'root' }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(StockStorageService, [{
        type: Injectable,
        args: [{
                providedIn: 'root'
            }]
